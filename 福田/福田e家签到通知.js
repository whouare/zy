...
const $ = new Env('福田e家签到通知')

// 本地账号配置
const localAccounts = `
`;

// 优先使用环境变量，如果没有则使用本地配置
const FTEJ = ($.isNode() ? process.env.FTEJ : $.getdata("FTEJ")) || localAccounts;

// 处理账号格式，移除多余空格和换行，过滤空行
let accounts = FTEJ.split(/[&\n]/).map(item => item.trim()).filter(item => item);

const FTEJ_PK = ($.isNode() ? process.env.FTEJ_PK : $.getdata("FTEJ_PK")) || '0'; // 开启=1，关闭=0
const FTEJ_Lottery = ($.isNode() ? process.env.FTEJ_Lottery : $.getdata("FTEJ_Lottery")) || '0'; // 开启=1，关闭=0
const FTEJ_SpringSign = ($.isNode() ? process.env.FTEJ_SpringSign : $.getdata("FTEJ_SpringSign")) || '1'; //春日活动
let notice = '';

async function main() {
    if (!accounts || accounts.length === 0) {
        console.log("未配置账号信息，请检查配置");
        return;
    }
    ...
    let finalNotice = results.join('');
    finalNotice += `\n📊 执行统计\n`;
    finalNotice += `   成功: ${successCount} 个\n`;
    finalNotice += `   失败: ${failCount} 个\n`;
    finalNotice += `\n⏰ ${$.time('MM-dd HH:mm:ss')}`;
...
async function springDaySign(memberID, memberComplexCode, phone, ticketValue, index) {
    try {
        const validateResponse = await request('/shareCars/validateToken.action', {
            ...
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                'Referer': `https://czyl.foton.com.cn/shareCars/activity/interactCenter250224/index.html?ftejMemberId=${memberID}&encryptedMemberId=${memberComplexCode}&channel=app`,
                'Cookie': `FOTONTGT=${ticketValue}`
            },
            body: `ticketName=FOTONTGT&ticketValue=${ticketValue.trim()}`
        });
        ...
        const signResponse = await request('/shareCars/c250224/sign.action', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
                'X-Requested-With': 'XMLHttpRequest',
...
async function springDayLottery(memberID, memberComplexCode, phone, ticketValue, index) {
    try {
        ...
        const lotteryResponse = await request('/shareCars/c250224/luckyDraw.action', {
            method: 'POST',
            headers: {
                'Accept': 'application/json, text/javascript, */*; q=0.01',
                'X-Requested-With': 'XMLHttpRequest',
            ...
            body: `encryptMemberId=${memberComplexCode}&activityNum=250224`
        });

        const lotteryMsg = lotteryResponse.data?.msg || '未知错误';
        console.log(`[${index}] 春日抽奖结果: ${lotteryMsg}`);

        if (lotteryMsg.includes('未中奖')) {
            console.log(`[${index}] 很遗憾，未中奖！`);
        }
...
async function processAccount(account, index) {
    try {
        ...
        if (!pointsInfo || pointsInfo.code !== 200) {
            return `❌ 账号 ${maskedPhone}\n   查询积分失败\n\n`;
        } else {
            return `✅ 账号 ${maskedPhone}\n   当前积分: ${pointsInfo.data.pointValue}\n\n`;
        }
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    return resolve('如果觉得没有朋友，就去找喜欢的人表白，对方会提出和你做朋友的。');  // 请求失败时返回默认文本
                } else {
                    await $.wait(2000);
                    resolve(data);
                }
                ...
                $.logErr(e, resp);
                resolve('如果觉得没有朋友，就去找喜欢的人表白，对方会提出和你做朋友的。');  // 异常时返回默认文本
            }
        });
    });
}
...
async function sendMsg(message) {
    if ($.isNode()) {
        ...
        try {
            ...
            console.log('发送通知失败，可能是缺少依赖模块');
            console.log('请确保以下文件/模块存在：');
            console.log('1. sendNotify.js');
            console.log('2. got (npm install got@11.8.6)');
            console.log('3. tough-cookie (npm install tough-cookie)');
            console.log('\n通知内容：\n' + message);
        }
    } else {
        $.msg($.name, '', message);
